...
class ProductTrader:
    ...
    def _log_state(self):
        ...
        if (
            self.wall_mid is not None
            and self.bid_wall is not None
            and self.ask_wall is not None
        ):
            ...
            for t in mkt:
                for bot_id, side in [(t.buyer, "buy"), (t.seller, "sell")]:
                    ...
                    entry = stats.setdefault(
                        bot_id, {"buy_qty": 0, "sell_qty": 0, "buys": 0, "sells": 0}
                    )
                    entry[f"{side}s"] += 1
    ...
    def take_orders(self, fv: float):
        for ask_price in list(self.asks):
            ...
            if ask_price <= fv and self.proj_position < 0:
                self._buy(ask_price, min(ask_vol, -self.proj_position), "flatten_buy")

        for bid_price in list(self.bids):
            bid_vol = self.bids[bid_price]
            if bid_price >= fv and self.proj_position > 0:
                self._sell(bid_price, min(bid_vol, self.proj_position), "flatten_sell")
    ...
    def make_orders(self, fv: float):
        ...
        prod_mem = self.memory.setdefault(self.PRODUCT, {})
        prod_mem["prev_make_bid"] = my_bid
        prod_mem["prev_make_ask"] = my_ask
...
class DynamicTrader(ProductTrader):
    ...
    def fair_value(self) -> Optional[float]:
        ...
        prod_mem = self.memory.setdefault(self.PRODUCT, {})
        prev_wall_mid = prod_mem.get("prev_wall_mid")

        if prev_wall_mid is None:
            return self.wall_mid

        return self.wall_mid - 0.5 * (self.wall_mid - prev_wall_mid)
    ...
    def get_orders(self) -> List[Order]:
        ...
        if fv is not None:
            ...
            prev_bid = prod_mem.get("prev_make_bid")
            prev_ask = prod_mem.get("prev_make_ask")
            if prev_bid is not None and self.max_buy > 0:
                self._buy(prev_bid, self.max_buy, "sticky_bid")
            if prev_ask is not None and self.max_sell > 0:
                self._sell(prev_ask, self.max_sell, "sticky_ask")

        if self.wall_mid is not None:
            prod_mem["prev_wall_mid"] = self.wall_mid
